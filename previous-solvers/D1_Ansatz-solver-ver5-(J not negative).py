...
def D1_ODE_system(t, yc):
    ...
    while n < N:
        ...
        while q < Q:
            ...
            summand2 = [Ham[m][n]*S[n][m].conjugate()*(y[m][Q].conjugate()*y[n][Q]) - Ham[m][n]*S[n][m]*(y[n][Q].conjugate()*y[m][Q])  for m in range(N)]

            term_1 = -1j*sum(summand1)
            term_2 = -1j*w[q]*y[n][q]
            term_3 =  1j*g[n][q]*w[q]
            term_4 = (-1j/2)*(y[n][q]/(y[n][Q].conjugate()*y[n][Q]))*sum(summand2)

            dlambdanq_dt[n][q] =   term_2 + term_3 + term_1 # + term_4
            #print(' t1 ',term_1,' t2 ',term_2,' t3 ',term_3,' t4 ',term_4)
            #print(' y[m][q]: ',y[m][q],' y[n][q]: ',y[n][q])
            q+=1
        n+=1
    ...
    print('lambda1:',y[0][500],'lambda2:',y[1][500])
    print('g1:',g[0][500],'g2:',g[1][500])
