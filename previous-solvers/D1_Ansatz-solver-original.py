...
def D1_ODE_system(t, yc):
    ...
    for n in range(N):
        for m in range(N):
            ...
            S[n][m] = cmath.exp(sum(summand)) #S[n][m] = cmath.exp(sum(summand))
    ...
    while n < N:
        q = 0
        while q < Q:
            summand1  = [Ham[n][m]*S[n][m]* ((y[n][Q].conjugate()*y[m][Q])/(y[n][Q].conjugate()*y[n][Q])) * (y[m][q]- y[n][q]) for m in range(N)] 
    ...
    while n < N:
        #first term
        summand1a  = [Ham[n][m]*S[n][m]*((y[n][Q].conjugate()*y[m][Q])/(y[n][Q].conjugate()*y[n][Q])) * sum([y[n][q].conjugate()*y[m][q] - y[n][q]*y[n][q].conjugate() for qq in range(0,Q)]) for m in range(N)] 
        summand1b  = [Ham[n][m]*S[n][m].conjugate()*((y[n][Q]*y[m][Q].conjugate())/(y[n][Q].conjugate()*y[n][Q])) * sum([y[n][q]*y[m][q].conjugate() - y[n][q]*y[n][q].conjugate() for qq in range(0,Q)]) for m in range(N)] 
        term_1 = (1j/2)*y[n][Q]*(sum(summand1a)+sum(summand1b))

        #second term
        summand2 = [Ham[n][m]*y[m][Q]*S[n][m] for m in range(N)]
        term_2 = -1j*sum(summand2)

        #third term
        summand3 = [g[n][qq]*w[qq]*(y[n][qq].conjugate() + y[n][qq]) for qq in range(Q)]
        term_3 = -1j*y[n][Q]*sum(summand3)#+1j*y[n][Q]*sum(summand3)

        #final result
        dalphan_dt[n] = term_1 + term_2 + term_3
...
def C_pp(n,omega):
    ...
    #summand = [(4*gamma**3)/(w[qq]**2 + gamma**2)**2 for qq in range(0,Q)]
    k = 1/sum(summand)
...
alpha_i = np.array([alpha_1, alpha_2]) #np.array([alpha_2, alpha_1])
...
#solve the ODE
sol = solve_ivp(D1_ODE_system, [0,t_f], y_i, t_eval = t, method='DOP853') #, method='DOP853', rtol = 1E-11, atol = 1E-14
...
#testing the correctness of the normalization
s1 = [g[0][qq]**2 for qq in range(0,Q)]
s2 = [g[1][qq]**2 for qq in range(0,Q)]

S1 = sum(s1)
S2 = sum(s2)

I = scipy.integrate.quad(HR_integrand, 0, np.inf)
