...
def spawn_g():
    q = 0
    while q < 1000:
        g[0][q] = (norm_g(0)*C_pp(w[q])/w[q]**2)**0.5
        g[1][q] = 0
        q+=1
    
    while q < Q:
        g[0][q] = 0
        g[1][q] = (norm_g(1)*C_pp(w[q])/w[q]**2)**0.5
        q+=1
    
    return 0

#spectral density function for excitonic polaron case
def C_pp(omega):
    #calculate spectral density
    C = (4*org_energy*(gamma**3)*omega)/(omega**2+gamma**2)**2
    return C

def HR_integrand(omega):
    return C_pp(omega)/(np.pi*omega**2)

#computes normalization constant k for coupling strengths
def norm_g(n):
    #calculate prefactor
    if n==0:
        summand = [C_pp(w[qq])/w[qq]**2 for qq in range(0,int(Q/2))]
    elif n==1:
        summand = [C_pp(w[qq])/w[qq]**2 for qq in range(int(Q/2),Q)]
    pf = 1/sum(summand)
